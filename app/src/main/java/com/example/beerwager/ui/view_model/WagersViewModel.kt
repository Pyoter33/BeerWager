...
class WagersViewModel @Inject constructor(
    ...
    private val _wagersState = MutableStateFlow(WagersState(searchText = savedStateHandle[NavigationArgs.ARG_SEARCH_QUERY] ?: ""))
    val wagersState = _wagersState.asStateFlow()

    init {
        setWagers(_wagersState)
    }
    ...
    private fun setWagers(
        ...
        filters: List<WagerCategory> = emptyList()
    ) {
        job?.cancel()
